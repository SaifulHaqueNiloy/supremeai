    ...
    async def generate_cost_forecast(self, days_ahead: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            forecast_result = {
                ...
                "trend": "increasing" if avg_daily_cost > daily_totals[-7 if len(daily_totals) >= 7 else 0] else "stable"
            }
        ...
        try:
            ...
            result = {
                ...
                "highest_cost_categories": sorted(
                    [("compute", avg_compute), ("storage", avg_storage), 
                     ("ai_models", avg_ai), ("network", avg_network)],
    ...
    async def _get_recent_cost_metrics(self, limit: int = 30) -> List[CostMetric]:
        ...
        try:
            ...
            for item in reversed(all_metrics[-limit:]):  # Most recent first
                recent_metrics.append(CostMetric(
    ...
    async def get_cost_optimization_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            highest_cost_cat = max(
                [("Compute", avg_compute), ("Storage", avg_storage), 
                 ("Network", avg_network), ("AI Models", avg_ai)],
                key=lambda x: x[1]
            )
            ...
            report = {
                ...
                "optimization_opportunities": {
                    ...
                    "top_3_potential_savings": round(
                        sum(min(3, len(opportunities)), key=lambda x: x.potential_savings, 
                           default=0) if opportunities else 0, 2
                    ),
