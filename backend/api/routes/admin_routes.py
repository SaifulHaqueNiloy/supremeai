...
def admin_firebase_totp_setup(payload: AdminFirebaseTotpSetupRequest):
    ...
    if db:
        ...
        if pending_secret and isinstance(pending_created, (int, float)) and (
            time.time() - float(pending_created)
        ) < _TOTP_PENDING_TTL_SECONDS:
            raise HTTPException(
                status_code=409,
...
async def admin_firebase_totp_verify(payload: AdminFirebaseTotpVerifyRequest, response: Response):
    ...
    temp_is_fresh = bool(temp_totp_secret) and isinstance(temp_created_at, (int, float)) and (
        time.time() - float(temp_created_at)
    ) <= _TOTP_PENDING_TTL_SECONDS
