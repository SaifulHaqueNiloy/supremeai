...
content = content.replace('@patch("core.security.api_key_middleware.AsyncRateLimiter.acquire", new_callable=AsyncMock)\n', "")
...
content = re.sub(r"    def test_rate_limit_exceeded.*?assert resp\.status_code == 429", rewrite_rl_test, content, flags=re.DOTALL)
