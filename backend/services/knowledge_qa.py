...
class KnowledgeQAService:
    ...
    def _is_allowed_document(
        self, metadata: dict[str, Any], tenant_id: str, role: str
    ) -> bool:
        ...
        if isinstance(allowed_data, dict):
            ...
            role_key = next(
                (k for k in allowed_data if k.lower() == role.lower()), None
            )
        ...
        allowed_roles = metadata.get(
            "allowed_roles", self.governance.get("allowed_roles", [])
        )
    ...
    def retrieve(
        ...
        return [
            ...
            and self._is_allowed_document(
                result[2].get("metadata", {}), tenant_id, role
            )
        ][:limit]
    ...
    async def answer(
        self, query: str, user: dict[str, Any], limit: int = 3
    ) -> dict[str, Any]:
        ...
        self._audit(
            tenant_id, user, query_hash, len(matches), "grounded answer returned"
        )
