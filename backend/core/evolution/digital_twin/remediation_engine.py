...
async def run_remediation_demo():
    ...
    logger.info("Remediation engine started, monitoring for issues...")
    ...
    logger.info(f"Restart action result: Success={restart_result.success}")

    # Scale up resources
    scale_result = await engine.manual_trigger_remediation("api_gateway", RemediationAction.SCALE_UP)
    logger.info(f"Scale up action result: Success={scale_result.success}")
