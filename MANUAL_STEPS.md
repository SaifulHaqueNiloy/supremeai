...
```bash
curl -s -o /dev/null -w "%{http_code}\n" ${RENDER_EXTERNAL_URL}/api/v1/health/live
...
| 7.2 | ~~Repoint or delete the dead `core/resilience/rollback_monitor.py` (targets Cloud Run revisions) to Render image rollback.~~ **✅ DONE (2026-08-30)** - Deleted dead code and related tests. Render has its own managed rollbacks. | ~~Needs Render API integration test~~ resolved |
| 7.3 | ~~Decide whether `/api/v1/evolution/forge` should require a human approval step before `SkillInstaller` (currently auto-promotes; AST+benchmark+sandbox gates pass, but ADR-0002 mandates HITL).~~ **✅ DONE (2026-08-30)** - Implemented `HITLEngine` to suspend deployments and added admin routes `/api/v1/hitl/pending` for manual approval/rejection. | ~~Product/security decision~~ resolved |
...
| 7.5 | ~~Move HITL/audit records to append-only storage (e.g. DB table + hash chain; `cryptographic_ledger.py` already provides the chaining logic) instead of 30-day Redis retention.~~ **✅ DONE (2026-08-30)** - Implemented `HITLAuditLedger` wrapping `CryptographicLedger` to persist an append-only hash chain to Firestore (`hitl_audit_ledger`). | ~~Storage design decision~~ resolved |
