...
import { capabilityFromModule, type UserCapability } from '../../types/contracts/capability';
...
export const UserDashboard: React.FC = () => {
  ...
  const capabilities: UserCapability[] = tools.map(capabilityFromModule);
  ...
  return (
    <main className="min-h-full bg-[var(--sa-canvas)] text-[var(--sa-ink)]">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-5 py-8 sm:px-8 lg:py-10">
        ...
        <section aria-labelledby="tools-heading"><div className="mb-4 flex items-center justify-between"><div><p className="sa-eyebrow">Your tools</p><h2 id="tools-heading" className="mt-1 text-xl font-semibold">Only what you need</h2></div><button type="button" onClick={() => setShowAddWizard(true)} className="inline-flex items-center gap-2 rounded-[var(--sa-radius-sm)] border border-[var(--sa-border)] px-3 py-2 text-xs font-medium hover:border-[var(--sa-primary)] hover:text-[var(--sa-primary)]"><Plus size={14} /> Add new</button></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{capabilities.map((item) => <ManageItem key={item.id} item={item} onManage={() => navigate(item.href || '/settings')} />)}</div></section>
      ...
      {showAddWizard && <AddNewWizard capabilities={capabilities} onClose={() => setShowAddWizard(false)} onAdd={(item) => { if (!enabledModules.includes(item.id as typeof enabledModules[number])) toggleModule(item.id as typeof enabledModules[number]); setShowAddWizard(false); }} />}
