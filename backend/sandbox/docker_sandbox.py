...
class DockerSandbox:
    ...
    def run_quarantine_test(self, staging_path: Path, entry_file: str, test_payload: str) -> Dict[str, Any]:
        ...
        cmd = [
            "docker", "run", "--rm",
            "--network", "none",                  # 🔒 নো নেটওয়ার্ক (Default-deny)
            "--memory", self.memory_limit,        # 📉 মেমরি ক্যাপ
            "--cpus", self.cpu_limit,             # 📊 সিপিইউ ক্যাপ
            "-v", f"{staging_path.resolve()}:/workspace:ro", # 📁 রিড-ওনলি মাউন্ট
            "-w", "/workspace",
            self.image_name,
            "python", "-c",
            f"import sys; import json; import {entry_file.replace('.py', '')} as tool; "
            f"print(json.dumps(tool.execute_tool({test_payload})))"
        ]
