...
- **ফুল পাথ টার্মিনাল কমান্ড (Full Path Terminal Commands):** যেকোনো টার্মিনাল কমান্ড (যেমন 
px, pnpm, 
pm, python, git) দেওয়ার সময় সবসময় সঠিক টার্গেট ডিরেক্টরি বা সম্পূর্ণ পাথ উল্লেখ করতে হবে (যেমন: F:\supremeai backup\apps\studio-client বা সম্পূর্ণ cd গাইডসহ), যাতে ইউজার বুঝতে পারেন ঠিক কোথায় কমান্ডটি চালাতে হবে।

- **Render ডিপ্লয়মেন্ট ফেইলিয়র লগিং (Render Deployment Failure Logging):** যেকোনো সময় Render ডিপ্লয় ফেইল করলে, প্রতিটি ফেইল্ড সার্ভিসের সম্পূর্ণ র (raw) লগ সংগ্রহ করে ender_deployment_failure_logs.md নামে একটি ডেসক্রিপ্টিভ মার্কডাউন ফাইল (Artifact) তৈরি করতে হবে।
...
- **শতভাগ প্রমাণভিত্তিক মূল-কারণ (Empirical Evidence Only):** কোনো অনুমান বা মাল্টি-অপশন ধারণা না দিয়ে লগের ওপর ভিত্তি করে সুনির্দিষ্ট একটি আসল কারণ খুঁজে বের করতে হবে।

- **ডকুমেন্টেশন ও স্বচ্ছতা:** যেকোনো ব্যর্থতায় র (raw) লগ সংগ্রহ করে তাৎক্ষণিক ডেসক্রিপ্টিভ ফাইল (Artifact) তৈরি করতে হবে এবং কোডবেসে বাংলায় স্পষ্ট কমেন্ট যুক্ত করতে হবে।

- **এন্ড-টু-এন্ড ভেরিফিকেশন:** যেকোনো কাজ শেষ ঘোষণা করার আগে লোকাল টেস্ট (pytest, API verification) রান করে এবং এনভায়রনমেন্ট অডিট করে ১০০% সুনিশ্চিত হতে হবে।
...
- **সর্বশেষ আদেশচালিত স্টেট নীতি (Last-Order Driven Stateful Rules):**
  - AGENTS.md-এর সকল বিপরীতমুখী কমান্ড বা অ্যাকশনের ক্ষেত্রে (যেমন: push start vs push stop, 	imer on vs 	imer off, explain/plan vs start work/implement) সবসময় ইউজারের **সর্বশেষ প্রদত্ত আদেশ (Last Order)** সক্রিয় থাকবে এবং নতুন বিপরীত আদেশ না আসা পর্যন্ত সেই স্টেট নিরবচ্ছিন্নভাবে কার্যকর থাকবে।
...
- **সমজাতীয় ও স্কোপ-ওয়াইড ভেরিফিকেশন নীতি (Homologous & Scope-Wide Verification Rule):**
  - কাজের প্রসেসের অন্যতম প্রধান অংশ: কোনো ফাইলে বাগ বা ব্রেকিং চেঞ্জ পাওয়া গেলে কেবল সেই একটি ফাইলে ফিক্স সীমাবদ্ধ রাখা যাবে না।
...
- **এন্ড-টু-এন্ড ডায়াগনস্টিক, ভেরিফিকেশন ও পুশ প্রোটোকল (End-to-End Diagnostic, Verification & Push Protocol):**
  - **ধাপ ১:** git remote -v চেক করে origin এবং 	arget পৃথকীকরণ।
  ...
  - **ধাপ ৭:** ইউজার সরাসরি "push" লিখলেই কেবল origin (SaifulHaqueNiloy/supremeai) এ পুশ করা। 	arget-এ সরাসরি পুশ সম্পূর্ণ নিষিদ্ধ।
