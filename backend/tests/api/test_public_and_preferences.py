...
def test_get_public_branding():
    response = client.get("/config/public/branding")
    assert response.status_code == 200
    data = response.json()
...
def test_get_preferences_offline_fallback():
    with patch("api.routes.preferences.db.client", None):
        response = client.get("/preferences/?user_id=test_user_123")
        assert response.status_code == 200
        data = response.json()
...
def test_upsert_preferences_offline():
    with patch("api.routes.preferences.db.client", None), \
        ...
        response = client.post("/preferences/?user_id=test_user_123", json=payload)
        assert response.status_code == 200
        data = response.json()
...
def test_get_preferences_with_db():
    ...
    with patch("api.routes.preferences.db.client", mock_db):
        response = client.get("/preferences/?user_id=u1")
        assert response.status_code == 200
...
def test_markdown_render_and_convert():
    sample_md = "# Hello SupremeAI\nThis is a test **bold** markdown."
    res = client.post("/api/markdown/render", json={"markdown": sample_md})
    assert res.status_code in [200, 404, 422]

    res_conv = client.post("/api/markdown/convert", json={"content": sample_md, "target_format": "html"})
    assert res_conv.status_code in [200, 404, 422]
