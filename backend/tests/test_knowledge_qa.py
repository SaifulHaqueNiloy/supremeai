...
def build_service():
    ...
    return KnowledgeQAService(vector_store=store, gateway=gateway, audit_logger=audit), store, gateway, audit
...
async def test_returns_grounded_answer_with_citation():
    ...
    store.add_document(
        ...
        {"tenant_id": "acme", "namespace": "supabase:vectors:company_policies", "source": "Remote Work Policy", "allowed_roles": ["standard_user"]},
    )

    result = await service.answer("How many remote days are allowed?", {"sub": "user-1", "tenant_id": "acme", "role": "standard_user"})

    assert result["grounded"] is True
    assert result["citations"] == [
        {"document_id": "remote-policy-1", "source": "Remote Work Policy", "chunk_index": None, "score": pytest.approx(0.3333, abs=0.01)}
    ]
...
async def test_cross_tenant_document_is_not_returned():
    ...
    store.add_document(
        ...
        {"tenant_id": "other-tenant", "namespace": "supabase:vectors:company_policies", "source": "Secret"},
    )

    result = await service.answer("confidential policy", {"sub": "user-1", "tenant_id": "acme", "role": "standard_user"})
...
async def test_prompt_injection_document_is_treated_as_data():
    ...
    store.add_document(
        ...
        {"tenant_id": "acme", "namespace": "supabase:vectors:company_policies", "source": "Leave Policy"},
    )

    await service.answer("What is the leave allowance?", {"sub": "user-1", "tenant_id": "acme", "role": "standard_user"})
