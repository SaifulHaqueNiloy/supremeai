...
class DynamicRepoManager:
    ...
    def prepare_workspace(self, target: TargetEntity) -> Path:
        """টার্গেট রেপোর জন্য আইসোলেটেড ওয়ার্কস্পেস প্রস্তুত ও আপডেট করে।"""
    ...
    def execute_git_commit(self, target_id: str, commit_message: str) -> str:
        """পারমিশন স্কোপ ভ্যালিডেট করার পর টার্গেট ওয়ার্কস্পেসে Commit নির্বাহ করে।"""
        ...
        subprocess.run(["git", "-C", str(target_dir), "add", "."], check=True, capture_output=True, text=True)
        res = subprocess.run(
