...
from models.meta_ai import PerformanceMetric
from models.meta_ai import SuggestionAction
from models.meta_ai import WeakestLinkReport
...
class PerformanceOracle:
    ...
    async def identify_weakest_links(
        ...
        if not agent_names:
            ...
            distinct_query = select(PerformanceMetric.agent_name).where(PerformanceMetric.recorded_at >= cutoff).distinct()
    ...
    def _suggest_action(
        ...
        if composite <= self._config.retrain_threshold:
            ...
            weakest = min(
                [("response_time", rt_pct), ("accuracy", acc_pct), ("cost", cost_pct), ("error_rate", err_pct)],
                key=lambda x: x[1],
            )
            return (
        ...
        return (
            ...
            f"LOW: Composite {composite:.2f}. " f"Marginal underperformance. Light optimization (prompt tuning, model swap) suggested.",
        )
