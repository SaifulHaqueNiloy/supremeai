...
class AutoScalingAgent:
    ...
    async def analyze_scaling_need(self, current_metrics: ResourceMetrics) -> ScalingRecommendation:
        ...
        try:
            ...
            if current_metrics.cpu_usage > policies["scale_up_thresholds"]["cpu"]:
                reasons.append(f"CPU usage ({current_metrics.cpu_usage}%) exceeds threshold ({policies['scale_up_thresholds']['cpu']}%)")
            elif current_metrics.cpu_usage < policies["scale_down_thresholds"]["cpu"]:
                reasons.append(f"CPU usage ({current_metrics.cpu_usage}%) below threshold ({policies['scale_down_thresholds']['cpu']}%)")
                
            if current_metrics.memory_usage > policies["scale_up_thresholds"]["memory"]:
                reasons.append(f"Memory usage ({current_metrics.memory_usage}%) exceeds threshold ({policies['scale_up_thresholds']['memory']}%)")
            elif current_metrics.memory_usage < policies["scale_down_thresholds"]["memory"]:
                reasons.append(f"Memory usage ({current_metrics.memory_usage}%) below threshold ({policies['scale_down_thresholds']['memory']}%)")
                
            if current_metrics.average_response_time > policies["scale_up_thresholds"]["response_time"]:
                reasons.append(f"Response time ({current_metrics.average_response_time}ms) exceeds threshold ({policies['scale_up_thresholds']['response_time']}ms)")
                
            if current_metrics.requests_per_second > policies["scale_up_thresholds"]["rps"]:
                reasons.append(f"Requests per second ({current_metrics.requests_per_second}) exceeds threshold ({policies['scale_up_thresholds']['rps']})")
            elif current_metrics.requests_per_second < policies["scale_down_thresholds"]["rps"]:
                reasons.append(f"Requests per second ({current_metrics.requests_per_second}) below threshold ({policies['scale_down_thresholds']['rps']})")
    ...
    def _determine_scaling_direction(self, metrics: ResourceMetrics, policies: Dict) -> int:
        ...
        """
        scale_up_triggered = (
            metrics.cpu_usage > policies["scale_up_thresholds"]["cpu"] or
            metrics.memory_usage > policies["scale_up_thresholds"]["memory"] or
            metrics.average_response_time > policies["scale_up_thresholds"]["response_time"] or
            metrics.requests_per_second > policies["scale_up_thresholds"]["rps"]
        )
        
        scale_down_triggered = (
            metrics.cpu_usage < policies["scale_down_thresholds"]["cpu"] and
            metrics.memory_usage < policies["scale_down_thresholds"]["memory"] and
            metrics.requests_per_second < policies["scale_down_thresholds"]["rps"]
        )
