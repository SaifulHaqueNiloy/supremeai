...
| 9 | ~~Ratify HITL approve-execution semantics (M17 P-B)~~ **✅ RESOLVED (#481) — canonical contract documented + enforced + regression-tested** | `docs/security/HITL_APPROVAL_CONTRACT.md` (supersedes `docs/plans/PENDING_APPROVALS.md` TASK-003) | Ratified state machine: `pending_approval → approved/rejected/expired` (S2) and `PENDING → APPROVED/REJECTED/CANCELLED → EXECUTED` (S1), default 24h TTL, terminal states absorb all further transitions (replay-proof), approve dispatches exactly once via the data-file dispatch-table (`services/hitl/dispatch.py`) with fail-closed pre-flip executor resolution, tenant-scoped + append-only hash-chain audit (`services/hitl/hitl_ledger.py`), explicit actor + tenant-match authorization gates, deterministic 403/404/409/410 error mapping. Regression suite: `backend/tests/hitl/test_hitl_approval_contract.py`. |
| 10 | ~~Canonicalize single `GITHUB_TOKEN` & purge `SUPREMEAI_GITHUB_TOKEN`~~ **✅ RESOLVED (2026-09-20)** | Render, Infisical, Local, Repo Configs | Replaced legacy/read-only tokens with unified Fine-Grained PAT (Issues R/W, PRs R/W, Contents R/W). Deleted `SUPREMEAI_GITHUB_TOKEN` from Render Node 4 (`supremeai-mcp-tower`), updated `GITHUB_TOKEN` across Infisical prod vault, Render, local `.env`, Desktop `env.txt`. Codebase unified (`auto_pr_pipeline.py`, `_audit.py`, `secrets_registry.yaml`). Live verified with `gh` CLI and issue resolution (#803). Full audit in `docs/security/GITHUB_TOKEN_CANONICALIZATION_PLAN.md`. |
...
```bash
curl -s -o /dev/null -w "%{http_code}\n" https://<render-backend-url>/api/v1/health/live
curl -s -o /dev/null -w "%{http_code}\n" https://<render-backend-url>/api/v1/health/ready
```
...
| 7.5 | ~~Move HITL/audit records to append-only storage~~ **✅ RESOLVED (2026-09-19)** — Enhanced `HITLAuditLedger` (`services/hitl/hitl_ledger.py`) to persist cryptographic SHA-256 hash chains into Supabase Postgres append-only table `hitl_audit_ledger` (with Firestore/local fallback), replacing 30-day ephemeral Redis TTL. | Resolved |
...
| 7.8 | ~~API keys: add a per-key `scopes` column if keys are ever meant to authorize routes~~ **✅ RESOLVED (2026-09-19)** — Verified `scopes` array column in Postgres schema (`models/api_key.py`, `api/routes/api_keys.py`, `api_key_middleware.py`), and added `require_api_key_scope` dependency in `api/dependencies.py` for route-level enforcement. | Resolved |
| 7.9 | **PARTIALLY RESOLVED (2026-09-19, issue #478):** fail-fast migration gate shipped — CI `migration-gate` job (offline chain verification always; live `alembic upgrade head` the moment the writer secret exists), readiness schema gate (`core/db_schema_gate.py`, 503 when required tables missing in prod), loud CRITICAL on skipped boot bootstrap. REMAINING OWNER STEP: Set `SUPABASE_DATABASE_URL_WRITER` env var in Render. This is the canonical writable Postgres endpoint (typically the direct connection string `postgresql://postgres.<project>:<password>@db.<project>.supabase.co:5432/postgres`, NOT the pooler at port 6543). Required so `pooled_pg.execute_ddl()` and `supabase_client.bootstrap_schema()` can create `automation_executions` table + `ai_memory` schema. If not set, both fall back silently to SQLite (data NOT durable across restarts) — same as today. | Infra config |
| 7.10 | **✅ RESOLVED (2026-09-19, issue #478):** `alembic upgrade head` wired into `ci-deploy-production.yml` as the `migration-gate` job that runs BEFORE `deploy-core` — offline SQL-plan verification proves the chain loads (exit 0, 2015 statements, single head `task_records_0001`); live upgrade activates automatically once `SUPABASE_DATABASE_URL_WRITER` is set; repo variable `MIGRATION_GATE_ENFORCE=true` flips missing-creds from amber warn to hard fail. Patch v4 added `automation_executions` to boot-time DDL as a stop-gap, but proper migration tooling is the long-term answer. | Infra wiring |
