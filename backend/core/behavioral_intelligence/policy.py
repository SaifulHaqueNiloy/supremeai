...
def review_behavioral_request(*, signals: Any, requested_action: str | None = None) -> BehavioralPolicyDecision:
    ...
    if requested_action:
        ...
        if action.mode is ActionMode.APPROVAL:
            return BehavioralPolicyDecision(True, "Behavioral signals may inform strategy; approval remains required.")
    risk = getattr(signals, "risk", None)
    if getattr(risk, "value", risk) == "high":
        return BehavioralPolicyDecision(True, "High-risk context requires conservative response and explicit uncertainty.", ("mental_state_claims", "sensitive_attributes"))
...
def sanitize_learning_metadata(metadata: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for key, value in metadata.items():
        ...
        if key_text not in allowed or any(term in key_text for term in ("prompt", "response", "content", "text")):
            continue
