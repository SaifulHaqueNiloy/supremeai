...
>
> **Amendment (2026-08-30, independent post-patch review):** AUD-1.4/1.5 downgraded from `[x]` to `[~]` — the shipped patch does not modify `agent_supervisor.py` and does not fix the known production shutdown bug (`CancelledError` re-raise, tracked as **AUD-1.10**). New rows AUD-1.7–1.10 and Phase 1b (AUD-M.1–M.7) added to give every previously prose-only `MANUAL_STEPS.md` item and the frontend-migration requirement its own trackable checklist row. Do not treat this patch as merge-ready until AUD-1.9 (green CI) and AUD-1.10 (#112 fix) are both `[x]`.
...
| AUD-1.4 | Verify SIGTERM/SIGINT graceful shutdown under Render | P1 | [~] | `main.py:72-88` handlers registered, no `sys.exit` (lifespan teardown preserved); wording/logging chain verified in `core/shutdown.py`. **Correction (2026-08-30, post-patch review): this patch does not touch `agent_supervisor.py` — 0 diff lines.** The known production bug (`agent_supervisor.shutdown_all()` re-raising `CancelledError` from the cancelled monitor task → shutdown sequence failing) is **NOT fixed**. Downgraded from `[x]` pending AUD-1.10. | 2026-08-30 |
| AUD-1.5 | Automated regression coverage for startup, shutdown, health | P1 | [~] | New `tests/core/test_main_entrypoint_guards.py` (5 tests: worker guard, reload policy, sigterm) + existing `test_lifespan.py` (6), `test_api_health.py` (5), `test_api_endpoints.py` (3) pass. `test_agent_supervisor_shutdown.py` covers existing behavior only — does not exercise the `CancelledError` regression path; not sufficient evidence to call shutdown coverage complete until AUD-1.10 lands. | 2026-08-30 |
...
| AUD-1.7 | Docker production build actually succeeds from current `main`/patch branch | P1 | [ ] | Not verified — Docker engine unavailable in sandbox. Requires a real Docker build (`MANUAL_STEPS.md` #1) before merge is considered production-safe. | — |
| AUD-1.8 | Deployed Render URL returns HTTP 200 on `/api/v1/health/live` post-deploy | P1 | [ ] | Only code-path + CI-boot-curl verified so far (AUD-1.1/0.7); a direct check against the live deployed service after this patch ships is still outstanding (`MANUAL_STEPS.md` #2). | — |
| AUD-1.9 | GitHub Actions CI run is green end-to-end on the patched branch | P0 | [ ] | All test-pass claims in this checklist are from an offline/sandbox run; no evidence yet of a real CI run (Postgres-backed) passing on this branch. Required before merge to `main`. | — |
| AUD-1.10 | Fix `agent_supervisor.shutdown_all()` re-raising `CancelledError` from a cancelled monitor task, causing shutdown failure (production finding #112) | P0 | [ ] | Confirmed NOT addressed by the 2026-08-30 remediation patch (`git diff` on `agent_supervisor.py` = empty). Needs a narrowly-scoped independent fix + dedicated regression test (`await monitor_task` wrapped to swallow the expected `CancelledError`) before this can be checked off. | — |
...
## Phase 1b — Post-Patch Manual Verification (tracked from `MANUAL_STEPS.md`)

**Goal:** Every item currently living only as prose in `MANUAL_STEPS.md` gets an explicit, checkable row so it can't be silently treated as done.

| ID | Finding | Severity | Status | Verification / Evidence | Date |
|----|---------|----------|--------|-------------------------|------|
| AUD-M.1 | Frontend clients updated for now-authenticated endpoints (markdown export UI, CI dashboard WS `?token=`, service-topology health-stream, `/agent/terminal-stream`) | P0 | [ ] | Breaking-by-design per `PATCH_README.md`/`MANUAL_STEPS.md` §7.7. No frontend changes included in this patch. Must be verified/shipped together, or the affected UI features will silently break for anonymous/unmigrated clients. | — |
| AUD-M.2 | Image signing (cosign) + SBOM published with release | P2 | [ ] | Artifact SHA-256 integrity gate exists; cosign/SBOM-in-release still MANUAL (`MANUAL_STEPS.md` #4). | — |
| AUD-M.3 | Backup/restore drill actually executed | P1 | [ ] | Policy doc written (AUD-5.7); drill execution itself not run (`MANUAL_STEPS.md` #6). | — |
| AUD-M.4 | Real canary traffic-splitting wired to `CanaryRolloutController` | P1 | [ ] | Controller logic + honest-reporting fix verified (AUD-6.6); actual traffic-splitting infra wiring is a MANUAL decision (`MANUAL_STEPS.md` #7). | — |
| AUD-M.5 | `/evolution/forge` human-approval requirement — final decision + owner | P0 | [!] | Flagged as open architecture decision since AUD-6.4; needs a named owner and deadline, not just a flag. | — |
| AUD-M.6 | Per-route `str(e)` response sweep (avoid leaking internals in error bodies) | P1 | [ ] | Tracked as residual hardening under AUD-2.9; no sweep performed yet. | — |
| AUD-M.7 | Rollback monitor re-pointed off dead Cloud-Run target | P1 | [ ] | Criteria/logic verified (AUD-6.7); the actual re-point task itself is still MANUAL. | — |
