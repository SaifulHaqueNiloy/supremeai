...
class KnowledgeQAService:
    ...
    def _authorize(self, user: dict[str, Any]) -> tuple[str, str]:
        ...
        if role not in set(self.governance["allowed_roles"]):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Role is not allowed to use this skill.")
    ...
    def retrieve(self, query: str, tenant_id: str, role: str, limit: int = 3) -> list[tuple[str, float, dict[str, Any]]]:
        ...
        return [
            ...
            if result[1] >= MIN_RETRIEVAL_SCORE and self._is_allowed_document(result[2].get("metadata", {}), tenant_id, role)
        ][:limit]
    ...
    async def answer(self, query: str, user: dict[str, Any], limit: int = 3) -> dict[str, Any]:
        ...
        if not answer:
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Knowledge answer generation is unavailable.")
    ...
    def _audit(self, tenant_id: str, user: dict[str, Any], query_hash: str, chunks_retrieved: int, reason: str) -> None:
        details = json.dumps(
            {
                "tenant_id": tenant_id,
