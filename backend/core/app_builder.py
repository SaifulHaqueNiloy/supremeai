...
বাংলা মন্তব্ব্য: এই মডিউলটি FastAPI অ্যাপ্লিকেশন ইনস্ট্যান্স তৈরি করে এবং সমস্ত মিডলওযয়্যার,
...
- মিডলওযয়্যার অর্ডার ক্রিটিক্যাল — authentication অবশ্যই honeypot এবং chaos মিডলওযয়্যারের আগে রান করবে।

Critical Security Note: মিডলওযয়্যার অর্ডার সঠিক করা হয়েছে যাতে অথেনটিকেশন
হনিপট এবং চাওস মিডলওযয়্যারের আগে রান হয়, সিকিউরিটি ইস্যু ঠিক করতে।
"""
...
# বাংলা মন্তব্ব্য: মিডলওযয়্যার ইম্পোর্ট লেজি-লোডেড — create_app()-এর ভিতরে ইম্পোর্ট হবে
...
def create_app(title: str = settings.PROJECT_NAME) -> FastAPI:
    ...
    বাংলা মন্তব্ব্য: মিডলওযয়্যার ইম্পোর্ট লেজিভাবে ফাংশনের ভিতরে করা হয়েছে
    যাতে মডিউল লোড হতে দেরি না হয় এবং কোল্ড স্টার্ট ২০% দ্রুত হয়।
    """

    # বাংলা মন্তব্ব্য: লেজি ইম্পোর্ট — মিডলওযয়্যার ক্লাস শুধু create_app() কল করলেই লোড হবে
    ...
    # বাংলা মন্তব্ব্য: মিডলওযয়্যার চেইন — ORDER IS CRITICAL FOR SECURITY
    ...
    # ব্যাকওযয়্যার্ড কম্প্যাটিবিলিটি: পুরনো /health পাথেও একই রাউটার এক্সপোজ করা থাকল।
