...
class PerformanceTuningAgent:
    ...
    async def analyze_performance_trends(self) -> List[OptimizationRecommendation]:
        ...
        try:
            ...
            if len(self.response_time_history) >= 10:
                ...
                historical_avg = sum(list(self.response_time_history)[:10]) / 10 if len(self.response_time_history) > 10 else recent_avg
    ...
    async def apply_optimization(self, optimization_name: str, parameters: Dict[str, Any] = None) -> PerformanceTuningResult:
        """
        ...
        try:
            ...
            logger.info(f"Applied optimization {optimization_name}: {status}, improvement: {improvement.get('overall_score', 0):.2f}")
            return result
        except Exception as e:
    ...
    def _calculate_performance_improvement(self, before: PerformanceMetric, after: PerformanceMetric) -> Dict[str, float]:
        ...
        try:
            ...
            cpu_score = max(0, min(1, (before.cpu_usage - after.cpu_usage) / before.cpu_usage)) if before.cpu_usage > 0 else 0
            memory_score = max(0, min(1, (before.memory_usage - after.memory_usage) / before.memory_usage)) if before.memory_usage > 0 else 0
            response_score = max(0, min(1, (before.response_time_ms - after.response_time_ms) / before.response_time_ms)) if before.response_time_ms > 0 else 0
            error_score = max(0, min(1, (before.error_rate - after.error_rate) / before.error_rate)) if before.error_rate > 0 else 0
            ...
            try:
                ...
                for rec in recommendations:
                    if rec.expected_impact == "high" and rec.confidence > 0.7:
                        await self.apply_optimization(f"{rec.component}_optimization", {
                            "recommendation": rec.recommendation,
