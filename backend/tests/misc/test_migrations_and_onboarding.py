...
class TestOnboardingAPI:
    ...
    async def test_complete_onboarding_endpoint(self):
        ...
        with (
            ...
            patch("api.routes.onboarding._save_user_preferences", return_value=True),
        ):
...
class TestViralReferralEngine:
    def test_generate_referral_code_format(self):
        ...
        with patch("tools.social.viral_referral_engine.db") as mock_db:
            ...
            with (
                patch.object(engine, "_save_local"),
                patch.object(engine, "_load_local", return_value={"codes": {}, "wallets": {}}),
            ):
