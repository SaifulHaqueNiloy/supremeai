    ...
    async def evolve(
        ...
        try:
            ...
            return EvolutionResult(
                ...
                mutations_applied=0,
                insights=[f"Evolution baseline preserved: {e!s}"],
            )
    ...
    def _create_chromosome_from_solution(self, solution: Any, fitness: float) -> Chromosome:
        ...
        if isinstance(solution, dict):
            for k, v in solution.items():
                genes.append(
                    Gene(gene_id=f"gene_{k}", gene_type="parameter", value=v, fitness=fitness, age=0, mutation_rate=0.1)
                )
        else:
            genes.append(
                Gene(gene_id="gene_core", gene_type="value", value=solution, fitness=fitness, age=0, mutation_rate=0.1)
            )
