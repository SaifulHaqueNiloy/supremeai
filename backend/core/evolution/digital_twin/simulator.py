...
async def run_sample_simulations():
    ...
    logger.info("Running sample service failure simulation...")
    failure_result = await simulator.simulate_service_failure(
        "llm_router", failure_type="complete", duration_minutes=10
    )
    logger.info(f"Failure simulation result: {failure_result.predicted_impact} impact")
    logger.info(f"Affected services: {len(failure_result.affected_services)}")
    logger.info(f"Recommendations: {len(failure_result.recommendations)}")
    ...
    logger.info(f"Traffic simulation result: {traffic_result.predicted_impact} impact")
    logger.info(f"Cascade effects: {len(traffic_result.cascade_effects)}")

    # Generate a report
    report = simulator.get_simulation_report(days_back=7)
    logger.info("\nSimulation report for last 7 days:")
    logger.info(f"Total simulations: {report['statistics']['total_simulations']}")
    logger.info(f"High/critical impacts: {report['statistics']['high_impact_count']}")
