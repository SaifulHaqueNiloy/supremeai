...
class HierarchicalMemoryTree:
    ...
    def add_branch(
        self, title: str, category: str = "general", tags: list[str] | None = None
    ) -> MemoryNode:
    ...
    def search_by_tag_or_category(
        ...
        for node in self.nodes.values():
            if (
                tag
                and tag.lower() in [t.lower() for t in node.tags]
                or category
                and node.category.lower() == category.lower()
            ):
                results.append(node)
        return results
