...
class BaseLearningStrategy(ABC):
    ...
    async def learn(
        self,
        event: LearningEvent,
        existing_knowledge: Optional[KnowledgeNode]
    ...
    async def learn(
        ...
        if existing_knowledge:
            ...
            existing_knowledge.confidence = (
                existing_knowledge.confidence * (1 - alpha) + 
                event.quality_score * alpha
            )
...
class UserPreferenceStrategy(BaseLearningStrategy):
    ...
    async def should_learn(self, event: LearningEvent) -> bool:
        return (
            event.event_type == LearningType.USER_PREFERENCE and
...
class ErrorCorrectionStrategy(BaseLearningStrategy):
    ...
    async def should_learn(self, event: LearningEvent) -> bool:
        return (
            ...
            event.event_type in [LearningType.ERROR_CORRECTION, LearningType.FEEDBACK]
        )
...
class UnifiedLearningEngine:
    ...
    async def learn(self, event: LearningEvent) -> Optional[KnowledgeNode]:
        """
    ...
    async def recall(self, query: LearningQuery) -> list[KnowledgeNode]:
        ...
        for node in self.knowledge_base.values():
            ...
            if (query_lower in node.pattern.lower() or 
                any(word in node.pattern.lower() for word in query_lower.split()[:3])):
                candidates.append(node)
        
        # Sort by confidence * recency
        candidates.sort(key=lambda n: (
            n.confidence * 
            (1 / (1 + (datetime.utcnow() - n.last_updated).total_seconds() / 86400))  # Time decay
        ), reverse=True)
    ...
    async def get_user_context(self, user_id: str, limit: int = 20) -> list[KnowledgeNode]:
        ...
        """
        query = LearningQuery(
            query_text="",  # Get all, not semantic search
            user_id=user_id,
            max_results=limit,
    ...
    async def _find_similar(self, event: LearningEvent) -> Optional[KnowledgeNode]:
...
_unified_learning_engine: Optional[UnifiedLearningEngine] = None
...
class SupremeLearningEngine:
    ...
    async def process_chat_message(self, query, user_id=None, **kwargs):
        ...
        context = await self._real.recall(LearningQuery(
            query_text=query,
            user_id=user_id,
            max_results=5
        ))
