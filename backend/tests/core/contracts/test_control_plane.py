...
def context():
    return ExecutionContext(tenant_id="tenant-a", actor_id="actor-a", workspace_id="workspace-a", correlation_id="corr-a", idempotency_key="key-a", capability="memory.write")
...
def test_realtime_client_payload_redacts_secrets():
    envelope = RealtimeEnvelope("execution", "updated", context(), 4, {"token": "secret", "status": "running"})
...
def test_frontend_request_has_correlation_and_redaction():
    request = frontend_request(context(), "task.cancel", {"authorization": "secret", "task_id": "task-1"})
