...
from core.orchestration.swarm_orchestrator import ExecutionResult, SwarmOrchestrator
...
def mock_external_calls():
    ...
    with patch(
        ...
        with patch(
            "core.mcp_client.MCPRegistryClient.discover_tools", new_callable=AsyncMock
        ) as mock_discover:
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        "core.orchestration.crew_departments.SwarmAgentBase._safe_skill_run",
                        new_callable=AsyncMock,
                    ) as mock_skill:
...
async def test_execute_task_general_intent(orchestrator, mock_external_calls):
    ...
    assert "mock_tool" in [
        t.get("name") for t in result.workspace.work_product.get("available_tools", [])
    ]
    ...
    assert any(
        "Multi-Agent DAG execution completed successfully" in log
        for log in result.workspace.execution_logs
    )
...
async def test_execute_task_code_generation_intent(orchestrator, mock_external_calls):
    ...
    with patch.object(
        orchestrator.agents["guardian"], "validate", new_callable=AsyncMock
    ) as mock_validate:
