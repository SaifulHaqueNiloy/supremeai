...
def demo_temporal_abstraction():
    ...
    logger.debug("Initializing Temporal Abstraction System...")
    ...
    logger.debug(f"Processed {len(temp_system.temporal_memory.events)} events")
    ...
    logger.debug(f"Created {len(hourly_abstractions)} hourly abstractions")
    ...
    logger.debug(f"Time span: {insights['time_span']['duration_hours']:.1f} hours")
    logger.debug(f"Total events: {insights['event_statistics']['total_events']}")
    logger.debug(f"Event types: {list(insights['event_statistics']['event_types'].keys())}")
    logger.debug(f"Detected patterns: {insights['detected_patterns']['total_patterns']}")
    logger.debug(f"Strong patterns: {len(insights['detected_patterns']['strong_patterns'])}")
    ...
    for granularity, abstractions in hierarchy.items():
        logger.debug(f"{granularity.capitalize()} level: {len(abstractions)} abstractions")
