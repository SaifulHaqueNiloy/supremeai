...
def estimate_signals(message: str, context: Mapping[str, Any] | None = None) -> BehavioralSignals:
    ...
    lower = text.lower()
    frustration = min(1.0, 0.35 + 0.2 * len(_FRUSTRATION.findall(lower))) if _FRUSTRATION.search(lower) else 0.0
    urgency = 0.75 if _URGENT.search(lower) else 0.15
    ambiguity = 0.65 if len(text.split()) < 4 or (" or " in lower and "?" not in text) else 0.1
    risk = SignalLevel.HIGH if any(word in lower for word in ("delete", "payment", "credential", "medical", "legal")) else SignalLevel.UNKNOWN
    goal = "debugging" if any(word in lower for word in ("bug", "error", "broken", "failed")) else "information"
    evidence = tuple(filter(None, ("frustration_signal" if frustration else "", "urgency_signal" if urgency > 0.5 else "", "short_or_ambiguous" if ambiguity > 0.5 else "")))
    return bounded_signals(BehavioralSignals(
        likely_goal=goal,
