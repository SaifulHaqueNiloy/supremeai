...
class AsyncRateLimiter:
    ...
    async def acquire(self, key: str, limit: int | None = None, window: int | None = None) -> bool:
        ...
        try:
            ...
            # Issue #460 (Pillar 1): ONE billable atomic EVAL per evaluation.
            # The old 4-command ZSET pipeline (zadd/zremrangebyscore/zcard/
            # expire) was billed as 4 separate Upstash ops — this single call
            # is billed once regardless of the script's internal commands.
            from core.cache.rate_limit_atomic import atomic_window_incr

            count = await atomic_window_incr(client, f"rate_limit:{key}", window)
