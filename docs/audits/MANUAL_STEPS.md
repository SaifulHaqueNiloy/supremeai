...
| 9 | Ratify HITL approve-execution semantics (M17 P-B) — **OPEN, founder ratification requested (2026-09-19)** | `docs/audits/PENDING_APPROVALS.md` TASK-003 | `HITLEngine.approve()` now EXECUTES approved actions via a data-file dispatch-table (`services/hitl/dispatch.py`) — previously approve was a silent status-flip (approved work never ran). Fail-closed on unknown targets; execution outcome loud in record + cryptographic ledger; state-machine intact (no double-execution). |
