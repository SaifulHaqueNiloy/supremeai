...
| **ERR-A03** | `frontend/src/components/customer/BrowserPreview.tsx` | Browser preview renders `<iframe src={currentUrl}>` directly in the DOM instead of proxying through backend Playwright. | ✅ FIXED (PR #399) — external HTTP URLs now render through the real backend Playwright pipeline (`/api/browser/automation`: session → navigate → base64 PNG screenshot → `<img data-testid="screenshot-proxy-view">`) with auto-proxy for consumers that only pass a `url` prop, verbatim error + "Open directly" fallback, and device presets. `allowDirectEmbed` remains an explicit opt-out for embeddable targets. | ✅ FIXED |
...
| :--- | :--- | :--- | :--- | :--- |
| **ERR-B02** | `/files`<br>`FilesPage.tsx`<br>`backend/api/routes/chat_upload.py` | Renders `WorkspaceModulePage module="files"` (which is undefined in `modules` record, falls back to blank). | ✅ FIXED (PR: err-b02-files-page) — new `FilesPage` with dropzone (`file-input` testid) + thumbnail explorer wired to the real `/api/chat/upload` contract (upload/list/serve/delete); backend gained a persistent LIST endpoint (disk scan + sidecar metadata, restart-safe) and sidecar cleanup on delete; 5 backend + 10 frontend tests. | ✅ FIXED |
...
| **ERR-B03** | `/activity`<br>`ActivityPage.tsx`<br>`frontend/src/services/activityService.ts` | Static mock cards describing activity features. | ✅ FIXED (PR: err-b03-activity-timeline) — new `ActivityPage` renders a real owner-scoped event timeline from the Mission Orchestration API (`GET /api/v1/missions`, missions/state_machine.py lifecycle: planned→approved→assigned→running→repairing→succeeded/failed/cancelled): state badges with icons, phase progress (current/total + done count), failure reason, repair count, priority, per-state filter chips, refresh, loading/error/empty states; 6 frontend tests. | ✅ FIXED |
| **ERR-B04** | `/runs`<br>`RunsPage.tsx`<br>`frontend/src/services/runService.ts` | Static mock cards describing execution tracking. | ✅ FIXED (PR: err-b04-runs-observer) — new `RunsPage` on the real Mission Orchestration API: execution list (state-filtered), per-run step observer (expandable trace viewer over `GET /api/v1/missions/{id}/trace` showing seq/phase/event/time), and state-machine-validated actions — **Retry (repair)** for failed runs, Start for approved/assigned, Cancel for active ones — with verbatim backend error surfacing; 7 frontend tests. | ✅ FIXED |
| **ERR-B05** | `/marketplace`<br>`MarketplacePage.tsx` | Static marketing card. Separate component `EnhancedSkillMarketplace.tsx` exists but is buried in admin. | ✅ FIXED (PR: err-b05-user-marketplace) — new user-facing `MarketplacePage` on the real `/api/skills/*` contract: full catalog, server-side keyword search (`/api/skills/search`), category filters derived from manifests, real install/uninstall flow (`installSkill`/`uninstallSkill` incl. persisted install state via `installed_only`), status badges, tags, loading/error/empty states; 7 page tests + existing service tests. | ✅ FIXED |
...
| :--- | :--- | :--- | :--- | :--- |
| **ERR-M01** | `scripts/find_stub_data.py` | Scans for only 20 literal regexes. Completely blind to `mock`, `fake`, `Math.random`, or canned responses. | ✅ FIXED (PR: err-m01-stub-scanner) — round 1 (10 new patterns: mock/fake/Math.random/canned/frozen-timestamp) landed earlier but stayed invisible: the blanket comment-skip in `scan_file()` made every comment-marker pattern a dead regex. Round 2 makes `stub_marker_comment` / `todo_replace_mock` / `simulate_saving_comment` comment-aware (they now fire, e.g. on kaggle_orchestrator's `# Task execution logic would go here`), broadens the marker regex to match prose between `#` and the marker, and excepts the markers in test files. 8 regression tests. CI wiring is tracked separately as ERR-M02 (report-only). | ✅ FIXED |
| **ERR-M02** | `.github/workflows/ci.yml` | The stub-blocker gate is not wired into GitHub Actions CI at all. | ✅ FIXED (PR: err-m02-ci-stub-gate) — round 1 wired `find_stub_data.py` as a **blocking** `stub-blocker` job (`--fail-on HIGH` over backend / frontend/src / scripts). Round 2 adds a **report-only** stub-trend step (MEDIUM+LOW, `continue-on-error`, artifact + step summary) so silent-failure counts trend monotonically downward, per the register synergy note (the `scripts/audit/system_defect_scan_2026_09_16.py` evidence generator stays available for manual/adhoc audits). | ✅ FIXED |
...
| **ERR-M05** | Remote Refs | ~320 stale remote branches fetched locally. | ✅ FIXED (PR: err-m05-branch-hygiene) — the count had already collapsed to 14 via branch-on-merge hygiene; on 2026-09-16 the 5 remaining merged-PR head branches (`feat/err-b01-project-spaces`, `feat/err-b02-files-page`, `docs/err-a03-register-sync`, `fix/err-m03-sentinel-unicode`, `fix/p0-critical-batch1`) were deleted via the GitHub API. 9 branches remain, each intentional: `main`, 2 open-PR heads (#414, #415), the 5-branch plan-governance stack (#400–#404, merged into each other, content NOT yet on main — deleting would lose it), and `fix/skills-install-uninstall-deploy` (closed-unmerged #385, superseded — kept for the maintainer to drop or restore). | ✅ FIXED |
...
* **`ERR-P02`: React Router Major Drift:** ✅ FIXED (2026-09-17, direct-to-main) — `react-router-dom` bumped `^6.30.6` → `^7.18.4`. The codebase was already prepared (`main.tsx` opted into `v7_startTransition`/`v7_relativeSplatPath` future flags); the now-removed `future` props were dropped from `main.tsx` + `App.test.tsx`. All 40 consumer files keep their `react-router-dom` imports (v7 ships it as a compat re-export). Verification: `tsc --noEmit` clean, **517/517 vitest tests pass**, production `vite build` succeeds.
...
| :--- | :--- | :--- | :--- | :--- |
| **ERR-F01** | **Execution State Machine** | No canonical `Run` model. Execution tracking is split across `automation_execution`, `execution_log`, and `pending_tasks`. | ⚠️ PARTIAL → production-wired (2026-09-17, direct-to-main): the canonical `Run` fabric (`backend/runs/`: models, strict 12-state machine, budgets, 8-class retry classification, `/api/v1/runs` API, 116 tests) landed earlier via PRs #341/#342/#344 but its evidence bridges had **zero production callers**. This change wires `runs.bridges.observe_automation_run` into the real dispatch path — `ExecutionRecorder.record_start` now creates the observing run (REQUESTED→POLICY_CHECKED→PLANNED→RUNNING, idempotent on `automation-obs:<event_id>`) and `record_completion`/`persist_execution` settle it to the mapped terminal state (DELIVERED→SUCCEEDED, FAILED→FAILED+error surface, SKIPPED→CANCELLED) — best-effort AFTER the authoritative commit, per Plan Section 10 (old DBs without the runs tables behave exactly as before). 8 new bridge regression tests (`tests/core/automation/test_execution_recorder_run_bridge.py`). Remaining follow-ups (test-only today): `pending_tasks`→WAITING_APPROVAL HITL observation and the dormant `execution_logs` writer. | ⚠️ PARTIAL (automation+orchestration paths production-observable; HITL/execution_logs bridges follow-up) |
| **ERR-F02** | **Memory Subsystem** | 15+ competing memory store implementations under `backend/memory/` (`chromadb`, `sqlite`, `hierarchical_tree`, `episodic`). | **M3: Memory Consolidation** (Retire duplicate stores; consolidate on `ai_memory` vector 384). | ❌ OPEN — remediation plan pinned 2026-09-17 (verified inventory, `backend/memory/`): the competing implementations are `chromadb_store.ChromaDBStore`, `sqlite_store.SQLiteMemoryStore` (with `supabase_store.SupabaseStore` subclassing it), `cloud_postgres_store.CloudPostgresStore` (+ its private `SQLiteStore`), `episodic_memory.EpisodicMemory`, `hierarchical_tree.HierarchicalMemoryTree`, `summary_tree`, `sliding_window.SlidingWindowMemory`, and `long_term_memory.MemoryManager`/`LongTermMemory`. **Consolidation order (extend-not-replace):** (1) declare the canonical store = Supabase `ai_memory` (vector 384) behind a `MemoryStore` protocol; (2) make `CloudPostgresStore`/`SupabaseStore` adapters of that protocol first (they share the backend); (3) re-point `long_term_memory.build_context()` consumers (e.g. `api/routes/chat.py` LTM block) at the canonical store; (4) freeze `ChromaDBStore`/`HierarchicalMemoryTree`/`summary_tree` behind a deprecation shim (no new callers) and migrate readers; (5) delete after a full release cycle with zero call-site hits (CI grep gate in the stub-gate job). Deliberately NOT started in this session: touching 8+ production call-sites without a live Supabase environment would be risky; the plan above is the M3 execution blueprint. | ❌ OPEN (blueprint pinned) |
| **ERR-F03** | **Context Assembly** | Unstructured prompt assembly with no token budgeting causes context bloat and provider rate limits. | ✅ FIXED (2026-09-17, direct-to-main): new `backend/context_engine/` package implements the M2 Context Engine — deterministic, pure-stdlib, smallest-sufficient-context assembly. Sections (system/memory/knowledge/history/user) are budgeted blocks: the user message is a hard reserve (never dropped), system is capped at 25% of budget, the rest fills priority-driven best-fit (a big low-priority block cannot starve smaller later blocks); budgets resolve per provider from `core.llm.token_budget.PROVIDER_TOKEN_BUDGETS` with a conservative 3 000-token default for unknown providers; every `assemble()` returns a full `BudgetReport` (per-section tokens, dropped ids, truncation flags). Wired into BOTH real call sites in `api/routes/chat.py` (`get_completion` + `stream_chat`) replacing the unbounded `f"{memory_ctx}{prompt}"` concatenation; prompt shape stays backward-compatible (user text remains the bare tail) so prompt-level caches are unaffected. 9 unit tests (`tests/context_engine/`); 4 existing chat-route tests still pass. | ✅ FIXED |
| **ERR-F04** | **Frontend Major Upgrades** | Breaking changes blocked on `react-router-dom` (v6→v7), `react-i18next` (v15→v17), `@storybook` (v8→v10). | ✅ FIXED (2026-09-17, direct-to-main): ① `react-router-dom` `^6.30.6` → `^7.18.4` — future-flag props removed (`main.tsx`, `App.test.tsx`), typecheck + 517/517 tests + production build all green (see ERR-P02). ② `react-i18next`/`i18next` — repo-wide scan proved BOTH packages were **dead dependencies** (zero imports; the app uses its own typed `src/hooks/useTranslation.ts` + `i18n/translations` layer), so they were removed instead of upgraded — the v15→v17 migration burden disappears entirely. ③ Storybook v8→v10 — resolved under ERR-P01 (v8-only packages dropped, `@storybook/react`/`react-vite` aligned to `^10.5.10`, `.storybook/main.ts` was already v10-only). | ✅ FIXED |
