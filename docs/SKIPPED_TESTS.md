...
| `test_verify_password_no_bcrypt` (×2 stacked decorators) / `test_verify_password_empty_hash` / `test_get_admin_credentials_missing_hash` / `test_get_admin_credentials_returns_hash` | `backend/tests/api/test_admin_routes.py:37,49,58,70` | "Needs update" — pre-`core/admin_routes` refactor assertions | DEFERRED-TICKET | Rewrite 4 tests against the current `_verify_password`/`_get_admin_credentials` contract; drop the duplicate stacked decorator. |
