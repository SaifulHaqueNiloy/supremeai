...
async def main():
    logger.info("Starting Phase 3 Load Test (1,000 Transactions)")
    ...
    with patch("litellm.acompletion", new_callable=AsyncMock) as mock_litellm:
        ...
        logger.info("\n=== Load Test Results ===")
        logger.info("Total Requests: 1000")
        logger.info(f"Success: {successes}")
        logger.info(f"Other Errors (Triggered SelfHealer): {errors}")
        logger.info(f"Total Time: {elapsed:.2f} seconds")
        logger.info(f"RPS: {1000 / elapsed:.2f} req/s")
        ...
        logger.info(f"Injected sandbox {sandbox_id} with age 11.6 minutes.")
