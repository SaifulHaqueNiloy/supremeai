...
class TestCommentThreadAI:
    ...
    async def test_summarize_thread_no_token(self):
        ...
        with patch.object(
            ai, "_get_pr_comments", new_callable=AsyncMock
        ) as mock_comments:
    ...
    async def test_webhook_responds_to_trigger(self):
        ...
        ai = CommentThreadAI(github_token="ghp_test")
        with patch.object(
            ai, "handle_pr_comment", new_callable=AsyncMock
        ) as mock_handle:
...
class TestTenantAdminAPI:
    ...
    async def test_list_tenants_empty(self):
        from api.routes.tenant_admin import _local_store, list_tenants
    ...
    async def test_create_tenant_applies_tier_defaults(self):
        from api.routes.tenant_admin import (
            TenantLimitCreate,
            _local_store,
            create_tenant,
        )
