...
def test_health_check_result_defaults():
    ...
    result = HealthCheckResult(status=HealthStatus.UNHEALTHY, message="Test message")
...
async def test_check_all_comprehensive():
    ...
    with (
        patch.object(checker, "check_application") as mock_app,
...
async def test_check_all_with_exception():
    ...
    with (
        patch.object(checker, "check_application", new_callable=AsyncMock) as mock_app,
        patch.object(
            checker, "check_redis", new_callable=AsyncMock, side_effect=Exception("Redis error")
        ),
