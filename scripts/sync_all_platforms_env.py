...
# একাধিক রিমোট প্ল্যাটফর্মে (GitHub, Render, Vercel, ইত্যাদি) স্বয়ংক্রিয়ভাবে ও একই সময়ে (Real-Time) সিঙ্ক এবং আপডেট করে।
...
def sync_github_secrets(env_dict):
...
def sync_render_env(env_dict):
    print("\n☁️ [2/3] Syncing environment variables to Render Web Services...")
    ...
    try:
        ...
        payload = [{'key': k, 'value': v} for k, v in env_dict.items() if not k.startswith('GITHUB_')]
...
def main():
    print("🚀 Starting Omni-Platform Real-Time Environment Secret Sync...")
    env_dict = load_clean_env()
    print(f"🔑 Loaded {len(env_dict)} total keys from .env")

    sync_github_secrets(env_dict)
    sync_render_env(env_dict)
    sync_vercel_env(env_dict)
