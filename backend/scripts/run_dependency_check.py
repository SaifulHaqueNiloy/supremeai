...
async def main():
    ...
    if pip_results.get("success") and pip_results.get("count", 0) > 0:
        ...
        logger.info("--- Outdated Pip Packages ---")
        logger.info(json.dumps(pip_results["outdated_packages"], indent=2))
    ...
    if pip_vuln_results.get("success") and pip_vuln_results.get("count", 0) > 0:
        ...
        logger.info("--- Pip Package Vulnerabilities (pip-audit) ---")
        logger.info(json.dumps(pip_vuln_results["vulnerabilities"], indent=2))
    else:
    ...
    if os.path.exists(os.path.join(frontend_path, "package.json")):
        ...
        if npm_results.get("success") and npm_results.get("count", 0) > 0:
            ...
            logger.info("--- Outdated NPM Packages ---")
            logger.info(json.dumps(npm_results["outdated_packages"], indent=2))
        else:
        ...
        if npm_vuln_results.get("success") and npm_vuln_results.get("audit_results"):
            ...
            logger.info("--- NPM Package Vulnerabilities (npm audit) ---")
            logger.info(json.dumps(npm_vuln_results["audit_results"], indent=2))
