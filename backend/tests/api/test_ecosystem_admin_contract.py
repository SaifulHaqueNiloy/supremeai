...
class TestProposalDecisionsContract:
    def test_decide_then_list_history(self, client: TestClient):
        ...
        history = client.get(
            f"/api/v1/ecosystem/admin/proposals/{proposal_id}/decisions"
        ).json()
        ...
        other = client.get(
            "/api/v1/ecosystem/admin/proposals/prop-does-not-exist/decisions"
        ).json()
        assert other == []
...
class TestAuthUsersContract:
    ...
    def test_set_role(self, auth_client: TestClient):
        res = auth_client.patch(
            "/api/v1/auth/users/viewer1/role", json={"role": "Operator"}
        )
        assert res.status_code == 200, res.text
