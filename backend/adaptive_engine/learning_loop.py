...
class ExperienceClusterer:
    ...
    def _generate_fingerprint(self, error_msg: str, action: str) -> str:
        ...
        for token in normalized.split():
            if any(kw in token for kw in ["error", "exception", "timeout", "fail", "denied", "invalid", "none"]):
...
class PerformanceDriftDetector:
    ...
    def detect_drift(self, provider: str) -> dict[str, Any] | None:
        ...
        return {
            ...
            "latency_change_pct": round((recent_lat_mean - old_lat_mean) / old_lat_mean * 100, 2) if old_lat_mean else 0.0,
