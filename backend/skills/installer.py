...
class SkillInstaller:
    ...
    def __init__(self, registry: SkillRegistry = None, skills_dir: str | None = None):
        self.registry = registry or (SkillRegistry() if SkillRegistry else None)
    ...
    def install_dependencies(self, dependencies: list[str]) -> bool:
        ...
        rejected = [
            d for d in dependencies if _dependency_base_name(d) not in ALLOWED_SKILL_DEPENDENCIES
        ]
        if rejected:
            logger.error(
