...
| [PLAN_004 — Letta/ChatGPT-style write-time memory distillation](PLAN_004_LETTA_STYLE_MEMORY_DISTILLATION_2026-09-17.md) | proposed | Replace the Eternal Brain write-path placeholders (`content[:200]`, `structure="{}"` — grep-verified) with opt-in LLM distillation on the existing zero-cost chain: distilled summary + structured facts in existing metadata JSON; wired to syncguard audit + memory API flag | 0 dep / 0 infra / 0 schema / 0 CI | Directly advances §1 Eternal Brain / §11 Memory-Must-Compound; complementary to PLAN_002 (in-session context) and to L4 lever (no store-topology consolidation here); Gate 5 offline retrieval eval required before any completion claim |
