"""Backend-root app assembly (moved from ``core/app.py`` — issue #683 Section 1).

The old ``core/app.py`` built the full ASGI app while importing
``api.routers`` / ``api.routes.stream_chat_sse`` — a forbidden core → api
inward layer violation. Assembling the app is composition-layer work (it must
see both ``core/`` and ``api/``), so the module now lives at the backend root
next to ``main.py``. ``core/app.py`` remains as a re-export shim so every
existing ``from core.app import app`` call site (including the uvicorn boot
...
# (also covers the legacy ``backend.app`` alias-import mode, see
# tests/api/test_router_mount_hygiene.py docstring).
...
# second registration. The registry is the single mount point; service-role
# filtering (scraper/worker intentionally skip admin surface) also now applies
# uniformly.
