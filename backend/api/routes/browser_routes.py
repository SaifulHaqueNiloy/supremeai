...
async def browser_ai_action(req: AIActionRequest):
    ...
    try:
        ...
        # (process cwd is backend/). Canonical module: core.llm.llm_gateway.
...
async def browser_security_scan(req: SecurityScanRequest):
    ...
    try:
        ...
        # FIX (import): canonical location is core.security.protection.ssrf_protection
...
async def save_browse_session(session_data: BrowseSessionRequest):
    ...
    """
    try:
        # FIX (import + api-drift): 'UnifiedMemory' never existed — the canonical
        # facade is UnifiedMemoryInterface (singleton unified_memory).
        ...
        # Store via the real Eternal Brain interface (embedding generated by
        # memory_service.save_memory underneath; sync call, cheap on free tier).
        unified_memory.store_long_term_memory(
            session_id=session_id,
            agent_type="browser",
            task_type="browse_session",
            content=session_data.url,
        ...
        return BrowseSessionResponse(
            success=True, session_id=session_id, message="Browse session saved to memory"
        )
...
async def get_browse_sessions(
    ...
    """
    try:
        # FIX (import + api-drift): rewritten against the real memory API.
        # retrieve_memories() returns ai_memory rows; we filter to
        # task_type='browse_session' client-side (small N on free tier).
        from core.unified_memory import unified_memory

        since_timestamp = int(time.time()) - (hours * 3600)

        rows = unified_memory.long_term_memory.retrieve_memories(user_id=userId)
        sessions = []
        for row in rows or []:
            if row.get("task_type") != "browse_session":
                continue
            ...
            sessions.append(
                {
                    "session_id": meta.get("session_id") or row.get("session_id"),
                    "url": meta.get("url"),
                    "userId": meta.get("userId") or row.get("user_id"),
                    "timestamp": ts,
        ...
        return {
            ...
            "sessions": sessions,
            "count": len(sessions),
            "query_params": {"limit": limit, "userId": userId, "hours": hours},
        }
