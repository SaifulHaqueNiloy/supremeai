...
class RoutingCriteria:
    ...
    # Constraints
    budget_constraint: Optional[float] = None  # Max cost in USD
    latency_requirement_ms: Optional[int] = None  # Max latency
...
class CostBasedStrategy(BaseRoutingStrategy):
    ...
    async def select_model(self, criteria, available_models, context=None):
        ...
        if criteria.budget_constraint:
            affordable = [m for m in available_models 
                        if m.cost_per_1k_tokens <= criteria.budget_constraint]
            if not affordable:
...
class PerformanceBasedStrategy(BaseRoutingStrategy):
    ...
    async def select_model(self, criteria, available_models, context=None):
        ...
        if criteria.latency_requirement_ms:
            fast_enough = [m for m in available_models 
                          if m.avg_latency_ms <= criteria.latency_requirement_ms]
            if not fast_enough:
...
class UnifiedRouter:
    ...
    async def route(
        ...
        available = self.registry.get_all(
            features=criteria.required_features,
            healthy_only=True
        )
        
        if not available:
            # Fall back to unhealthy models
            available = self.registry.get_all(features=criteria.required_features, healthy_only=False)
        ...
        selected, fallbacks = await strategy_impl.select_model(
            criteria, 
            available,
            context={"user_id": criteria.user_id}
        )
...
class ModelRouter:
    ...
    async def route_and_generate(self, prompt, task_type="general", **kwargs):
        ...
        decision = await self._real.route(RoutingCriteria(
            prompt=prompt,
            task_type=task_type,
            **kwargs
        ))
