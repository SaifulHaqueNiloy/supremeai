...
class TestGovernedExecutor(unittest.TestCase):
    def test_emits_start_and_terminal_events(self):
        ...
        capability = Capability(name="Echo", purpose="test", signature="test.governed.v1", lifecycle_state=CapabilityLifecycleState.ACTIVE)
        bridge.register(capability, lambda request: request["payload"])
        events = []
        executor = GovernedCapabilityExecutor(bridge, lambda kind, payload: events.append((kind, payload)))
        result = asyncio.run(executor.execute("test.governed.v1", NodeContext("t", "a", "c", "test.governed.v1"), {"ok": True}))
        self.assertTrue(result.ok)
        self.assertEqual([event[0] for event in events], ["execution.started", "execution.succeeded"])
